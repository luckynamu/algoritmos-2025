...
const int maxCiudades = 1000;

struct Localidades{
    char localidad[40];
    Ciudades vectorCiudades[maxCiudades];
    int cantCiudades;
    int corredoresTotales;
    int tiempoSegundos;
};
...
void cargarCorredoresCiudad(FILE* archivo, CorredoresCiudad vector[], int &longitud);
...
int main(){
    ...
    FILE *archivoCiudades = fopen("C:/Users/mvpas/Documents/Universidad/Algoritmos y Estructuras de Datos/Archivos de T.P/Ciudades.bin", "rb");
    if (!archivoCiudades){
    ...
    CorredoresCiudad vectorCorredoresCiudad[2000];
    int cantidadCorredoresCiudades = 0;
    cargarCorredoresCiudad(archivoCiudades, vectorCorredoresCiudad, cantidadCorredoresCiudades);

    //Paso 15: Buscamos los corredores del archivo original que coincidan con los numeros del archivo Ciudades.
    Localidades vectorLocalidades[cantidadCorredoresCiudades];
...
void cargarCorredoresCiudad(FILE* archivo, RegCorredores vectorCorredores[], int longitudCorredores, CorredoresCiudadInfo vector[], int &longitud){
    CorredoresCiudad corredores;
    longitud = 0;
    while(fread(&corredores, sizeof(CorredoresCiudad), 1, archivo) == 1){
        for(int i = 0; i < longitudCorredores; i++){
            if(vectorCorredores[i].numero == corredores.numero){
                strcpy(vector[longitud].localidad, vectorCorredores[i].localidad);
                strcpy(vector[longitud].ciudad, corredores.ciudad);
                strcpy(vector[longitud].tiempoTotal, vectorCorredores[i].llegada);
                longitud++;
                break;
            }
        }
